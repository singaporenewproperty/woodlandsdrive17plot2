(function ($) {
    $.fn.dreamimage = function (tempName) {
        var cs, x = 0;
        if ($("#dreambackground")[0] == undefined) {

            $("body").append('<div id="dreamland" style="position: fixed;z-index:214748364855555 !important; padding: 0px;display: none;top:0px;left:0px;"><div id="dreambackground" style="position: fixed;height: 100%;z-index:1000;width: 100%;background-color: rgba(0, 0, 0, 0.7);"></div><div id="tool"><svg height="50" width="50" style="cursor:pointer;float: right;z-index: 10005;position: fixed; right:10px;"><rect x="0" y="0" height="50" width="50" fill="white" style="display: none;" /><path d="M 5 5 L 45 45" stroke="white" stroke-width="1" /><path d="M 45 5 L 5 45" stroke="white" stroke-width="1" /></svg><span style="width: 120px;background-color: #555;color: #fff;text-align: center;padding: 5px 0;border-radius: 6px;position: absolute;z-index: 100000;left:50px;top:10px;display:none;">Close</span></div><div id="dreamcontent" style="text-align: center;height: 95%;width: 95%;z-index:10001;padding: 40px;position: fixed;"><img src="#" style="transform: rotate(0deg);max-height: 90%;max-width: 90%;width: auto;height: auto;position: relative;box-shadow: 0 0 25px #111111;" alt="Unable to load image"></img></div><h2 id="sub" style="font-family: sans-serif;font-weight: 200;display: none;z-index: 10009;text-align:center;position: fixed;text-shadow: 2px 2px 7px black;color: white;bottom: 0px;width: 100%;text-align: center;"></h2>');

            /*$("body").append('<div id="dreamland" style="position: fixed;z-index:214748364855555 !important; padding: 0px;display: none;top:0px;left:0px;"><div id="dreambackground" style="position: fixed;height: 100%;z-index:1000;width: 100%;background-color: rgba(0, 0, 0, 0.7);"></div><div id="tool"><svg height="50" width="50" style="cursor:pointer;float: right;z-index: 10005;position: fixed; right:10px;"><rect x="0" y="0" height="50" width="50" fill="white" style="display: none;" /><path d="M 5 5 L 45 45" stroke="white" stroke-width="1" /><path d="M 45 5 L 5 45" stroke="white" stroke-width="1" /></svg><span style="width: 120px;background-color: #555;color: #fff;text-align: center;padding: 5px 0;border-radius: 6px;position: absolute;z-index: 100000;left:50px;top:10px;display:none;">Close</span></div><div id="dreamcontent" style="text-align: center;height: 95%;width: 95%;z-index:10001;padding: 40px;position: fixed;"><img src="#" style="transform: rotate(0deg);max-height: 90%;max-width: 90%;width: auto;height: auto;position: relative;box-shadow: 0 0 25px #111111;" alt="Unable to load image"></img></div><h1 id="sub" style="font-family: sans-serif;font-weight: 200;display: none;z-index: 10009;text-align:center;position: fixed;text-shadow: 2px 2px 7px black;color: white;bottom: 0px;width: 100%;text-align: center;"></h1>');*/
        ...
        $(document).on('click', '.pop-up-btn', function () {
            var _this = $(this).closest('.pop-up-img-card').find('.img-popup');
            cs = $("body").css("overflow");
            $("body").css("overflow", "hidden");
            $("#dreamland > #sub").hide();
            if ($(_this).attr("img-large") == undefined)
                var src = $(_this).attr("src");
            else
                var src = $(_this).attr("img-large");
        ...
        $(document).on('click', '.floor-pop-up-btn, .floorplan-img', function () {
            var _this = $(this).closest('.pop-up-img-card').find('.floorplan-img');
            var src = $(_this).attr("img-large") || $(_this).attr("src");
            var title = $(_this).attr("title");
            ...
            // Use a default title if none is provided, and ensure it is not set to the src
            if (!title) {
                title = "Floor Plan";
            }
            ...
            $(".floorplan-popup .popup-header").text(title);
            $("#floorplan-image").attr("src", src);

            // Show pop-up
            $(".floorplan-popup").removeClass('d-none');
            $("body").css("overflow", "hidden");
        });

        // Close pop-up when clicking on the close button
        $(document).on('click', '.close-popup', function () {
            $(".floorplan-popup").addClass('d-none');
            $("body").css("overflow", "auto");
        });

        // Close pop-up when clicking outside the image (on the overlay)
        $(document).on('click', '.floorplan-popup', function (e) {
            if ($(e.target).is('.floorplan-popup')) {
